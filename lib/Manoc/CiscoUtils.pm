...
sub get_pwds {
    ...
    if(!$telnet_pwd || !$enable_pwd){
        my $conf_file = "/etc/manoc.conf";
        -f $conf_file or $conf_file = File::Spec->catfile(Manoc::Utils::get_manoc_home(), 'etc', 'manoc.conf');
        -f $conf_file or return(undef, undef);
        $Conf = new Config::Simple($conf_file);
        $telnet_pwd = $Conf->param('password.telnet') or return(undef, undef);
        $enable_pwd = $Conf->param('password.enable') or return(undef, undef);
    }
...
sub login_device {
    ...
    eval {
        ...
        $enable_pwd and 
	    $session->enable($enable_pwd) or return(undef);
    };
    $@ and return(undef);
...
sub switch_port_status {
    ...
    ($telnet_pwd and $enable_pwd) or return (0, "Impossible retrieve device passwords to login");
    ...
    eval {
        $session->cmd("configure terminal")  or return(0, "Impossible configure interface $iface_id");
        $session->cmd("interface $iface_id") or return(0, "Invalid interface: $iface_id");
        if ($status eq "up") {
            $session->cmd("shutdown")        or return(0, "Impossible configure interface $iface_id");
        } else {
            $session->cmd("no shutdown")     or return(0, "Impossible configure interface $iface_id");
        }
    };
...
sub get_config {
    ...
    eval {
        @config_arr = $session->cmd("show running");
    };
